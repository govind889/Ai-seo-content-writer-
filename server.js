...
if (!JWT_SECRET || JWT_SECRET.length < 32 || JWT_SECRET === "dev-secret-change-me") {
  throw new Error(
    "JWT_SECRET must be set to a strong secret (at least 32 characters). Update your .env before starting the server."
  );
}
...
async function openAiSeoContent(input) {
  ...
  const text = fromOutputText || fromOutputArray;
...
async function generateSeoContent(payload) {
  if (OPENAI_API_KEY) {
    try {
      const text = await openAiSeoContent(payload);
  ...
  return { text: fallbackSeoContent(payload), source: "fallback" };
}
...
app.post("/api/content/generate", authRequired, async (req, res) => {
  ...
  const generation = await generateSeoContent({ keyword, audience, tone, intent, language, length, includeFaq });
  ...
  const insert = db
    ...
    .run(user.id, keyword, audience || null, tone || null, intent || null, language || null, length || null, generation.text);
